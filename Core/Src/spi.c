...
void HAL_SPI_MspInit(SPI_HandleTypeDef* spiHandle)
{
  ...
  /* USER CODE END SPI1_MspInit 0 */
    ...
    PA15     ------> SPI1_NSS
    */
    GPIO_InitStruct.Pin = AS5047_SCK_Pin|AS5047_MISO_Pin|AS5047_MOSI_Pin;
    ...
    GPIO_InitStruct.Pin = AS5047_NSS_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    GPIO_InitStruct.Alternate = GPIO_AF5_SPI1;
    HAL_GPIO_Init(AS5047_NSS_GPIO_Port, &GPIO_InitStruct);
    ...
    /* SPI1_TX Init */
    hdma_spi1_tx.Instance = DMA2_Channel5;
...
void HAL_SPI_MspDeInit(SPI_HandleTypeDef* spiHandle)
{
  ...
  /* USER CODE END SPI1_MspDeInit 0 */
    ...
    PA15     ------> SPI1_NSS
    */
    HAL_GPIO_DeInit(GPIOA, AS5047_SCK_Pin|AS5047_MISO_Pin|AS5047_MOSI_Pin|AS5047_NSS_Pin);
...
void SPI_TransmitReceive_DMA(uint16_t* transferData, uint16_t* receiveData, uint16_t size)
{
  for (uint16_t i=0; i<size; i++)
  {
    ...
    uint8_t res = HAL_SPI_TransmitReceive_DMA(&hspi1, (uint8_t*)(transferData+i), (uint8_t*)(receiveData+i), 1) ;
...
void SPI_Transfer_DMA(uint16_t* transferData, uint16_t size)
{
  for (uint16_t i=0; i<size; i++)
  {
    ...
    uint8_t res = HAL_SPI_Transmit_DMA(&hspi1, (uint8_t*)(transferData+i), 1) ;
    if(res!= HAL_OK) // after we will use bytesize if we want to optimize
        res = res;
...
void SPI_Receive_DMA(uint16_t* receiveData, uint16_t size)
{
  for (uint16_t i=0; i<size; i++)
  {
    ...
    uint8_t res = HAL_SPI_Receive_DMA(&hspi1, (uint8_t*)(receiveData+i), 1) ;
    if(res!= HAL_OK) // after we will use bytesize if we want to optimize
        res = res;
    // LL_DMA_DisableChannel(DMA2, LL_DMA_CHANNEL_4);
    // LL_DMA_ConfigAddresses(DMA2, LL_DMA_CHANNEL_4, LL_SPI_DMA_GetRegAddr(SPI1), (uint32_t)(receiveData+i), LL_DMA_GetDataTransferDirection(DMA2, LL_DMA_CHANNEL_4));
